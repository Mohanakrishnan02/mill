  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <nav className="mb-6 text-sm text-stone-500">
        <Link href="/" className="hover:text-[#2F6B3A]">Home</Link>
        {" / "}
        <Link href="/products" className="hover:text-[#2F6B3A]">Products</Link>
        {" / "}
        <span className="text-stone-800">{product.name}</span>
      </nav>

      <div className="grid gap-10 lg:grid-cols-2">
        <div className="relative aspect-square overflow-hidden rounded-xl bg-gradient-to-br from-[#FAF6EB] to-[#F5EDD8]">
          <ProductImage
            src={product.image}
            alt={product.name}
          ...
          {prev && slug !== "jgl" && (
            <Link
              ...
              className="absolute left-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/80 bg-white/90 text-[#3D3428] shadow-md transition hover:border-[#D4A017] hover:text-[#D4A017]"
            >
          ...
          {next && (
            <Link
              ...
              className="absolute right-3 top-1/2 flex h-10 w-10 -translate-y-1/2 items-center justify-center rounded-full border border-white/80 bg-white/90 text-[#3D3428] shadow-md transition hover:border-[#D4A017] hover:text-[#D4A017]"
            >
        ...
        <div>
          <p className="text-sm font-medium text-[#D4A017]">{product.category}</p>
          <h1 className="mt-1 text-3xl font-bold text-[#3D3428]" style={{ fontFamily: "var(--font-yeseva)" }}>
            {product.name}
          </h1>
          {product.tamil && (
          ...
          <div className="mt-6 flex items-end gap-3">
            ...
            {discount > 0 && (
              <span className="rounded bg-[#E8F5EE] px-2 py-0.5 text-sm font-bold text-[#2F6B3A]">
                {discount}% off
              </span>
            )}
          </div>
                ...
                <button
                  ...
                  className={`rounded border px-4 py-2 text-sm font-semibold transition ${
                    selectedVariantId === v.id
                      ? "border-[#D4A017] bg-[#F5E9C0] text-[#D4A017]"
                      : v.inStock
                        ? "border-stone-200 text-stone-700 hover:border-[#D4A017]"
          ...
          <div className="mt-8 flex gap-3">
            <button
              ...
              className="flex-1 rounded border-2 border-[#D4A017] bg-[#F5E9C0] py-3.5 text-sm font-bold text-[#1E4D2B] hover:bg-[#D4A017] hover:text-[#14261C] disabled:opacity-40"
            >
              ADD TO CART
            </button>
            <button
              ...
              disabled={!variant.inStock}
              className="flex-1 rounded bg-[#D4A017] py-3.5 text-sm font-bold text-[#14261C] hover:bg-[#E8C547] disabled:opacity-40"
            >
              BUY NOW
            </button>
          </div>
          ...
          <ul className="mt-8 space-y-3 border-t border-stone-100 pt-6">
            ...
            <li className="flex items-center gap-2 text-sm text-stone-600">
              <Shield className="h-4 w-4 text-[#2F6B3A]" />
              100% traditional — stone-milled, no chemicals
            </li>
            <li className="flex items-center gap-2 text-sm text-stone-600">
              <Check className="h-4 w-4 text-[#2F6B3A]" />
