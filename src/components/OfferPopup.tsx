...
export function OfferPopup() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="relative overflow-hidden bg-gradient-to-br from-[#2e7d32] via-[#388e3c] to-[#1b5e20] px-5 pb-8 pt-5 text-white">
          ...
          <div className="offer-badge-pop relative inline-flex items-center gap-1.5 rounded-full border border-[#f5a623]/40 bg-black/20 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#f5a623]">
        ...
        <div className="relative -mt-5 mx-4">
          <div className="offer-featured overflow-hidden rounded-xl border-2 border-[#f5a623] bg-gradient-to-br from-[#fff8e1] to-white p-4 pb-8 shadow-lg">
              ...
              {offers.map((offer, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div>
                      <p className="text-sm font-extrabold text-[#5d3a1a]">{offer.title}</p>
            ...
            <div className="absolute bottom-3 left-0 right-0 flex justify-center gap-1.5">
              {offers.map((_, i) => (
                <span
                  key={i}
                  className={`h-1.5 rounded-full transition-all duration-300 ${i === activeSlide ? "w-5 bg-[#e07b00]" : "w-1.5 bg-stone-300"}`}
                />
              ))}
            </div>
          </div>
        ...
        <ul className="space-y-2 px-4 py-4">
          {offers.map((offer, i) => {
            ...
            return (
              <li
                key={offer.title}
                className="offer-row flex items-center gap-2.5 rounded-lg border border-stone-100 bg-[#fdf8f0] px-3 py-2"
                style={{ animationDelay: `${0.15 + i * 0.08}s` }}
              >
                <Icon className="h-4 w-4 shrink-0 text-[#e07b00]" />
        ...
        <div className="border-t border-stone-100 px-4 pb-5 pt-3">
          <Link
            href="/products"
            onClick={dismiss}
            className="offer-cta block w-full rounded-lg bg-gradient-to-r from-[#e07b00] to-[#f5a623] py-3 text-center text-sm font-extrabold text-white shadow-md"
          >
            🛒 Shop & Grab Offers
          </Link>
          <button
