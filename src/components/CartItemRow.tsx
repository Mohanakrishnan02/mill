...
export function CartItemRow({ item, variant = "cart" }: CartItemRowProps) {
  ...
  return (
    <div className="flex gap-3 border-b border-stone-100 py-4 last:border-0">
      <div className="relative h-16 w-16 shrink-0 overflow-hidden rounded bg-[#F5EDD8]">
        <ProductImage src={item.image} alt={item.name} className="object-cover" sizes="64px" />
      </div>
      <div className="min-w-0 flex-1">
        <Link
          href={`/products/${item.productSlug}`}
          className="line-clamp-2 text-sm font-bold text-stone-800 hover:text-[#2F6B3A]"
        >
          {item.name}
        </Link>
        {item.tamil && (
        ...
        {variant === "cart" ? (
          <div className="mt-2 flex flex-wrap items-center gap-3">
            ...
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium text-stone-500 hover:text-[#2F6B3A]"
            >
          ...
          <div className="mt-2 flex flex-wrap items-center gap-3">
            <button
              ...
              className="rounded border border-[#2F6B3A] px-3 py-1 text-xs font-semibold text-[#2F6B3A] hover:bg-[#E8F5EE]"
            >
              Move to cart
            </button>
            <button
